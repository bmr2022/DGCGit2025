...
// ======================================================
// DrilldownManager — Full (paste / replace existing manager)
// ======================================================
var DrilldownManager = (function () {
    const FILTER_KEY = "DRILLDOWN_FILTERS_";
    const NAV_STACK_KEY = "DRILLDOWN_STACK";
    ...
    function saveFilters(formName, filters) {
        if (!formName || !filters) return;
        sessionStorage.setItem(FILTER_KEY + formName, JSON.stringify(filters));
    }

    function restoreFilters(formName) {
        const raw = sessionStorage.getItem(FILTER_KEY + formName);
        if (!raw) return null;
        try { return JSON.parse(raw); }
        catch (e) {
            console.error("DrilldownManager.restoreFilters parse error:", e);
            return null;
        }
    }

    function pushNavigation(navInfo) {
    ...
    function getLastNextForm() {
        const stack = getStack();
        if (!stack.length) return null;
        return stack[stack.length - 1].nextForm || null;
    }

    function restoreIfAvailable(formName) {
        const filters = restoreFilters(formName);
        if (!filters) return;
        // this does minimal restore (used on same-page restore)
        Object.keys(filters).forEach(key => {
            const $el = $("#" + key);
            if (!$el.length) return;
            if ($el.attr("type") === "checkbox") {
                $el.prop("checked", !!filters[key]);
            } else if ($el.hasClass("select2") || $el.hasClass("custom-select2")) {
                $el.val(filters[key]).trigger("change.select2");
            } else if (key === "FromDate" || key === "ToDate") {
                $el.datepicker("setDate", filters[key]);
            } else {
                $el.val(filters[key]).trigger("change");
            }
        });
    }

    // ✅ goBack: pop current, ensure prev filters exist, navigate to prev.url
    function goBack() {
        const stack = getStack();
        if (!stack || stack.length < 2) {
            console.log("DrilldownManager.goBack: no previous entry");
            return;
        }
        ...
        // Ensure filters exist for prev.currentForm (best-effort)
        const prevFiltersKey = FILTER_KEY + prev.currentForm;
        const prevFiltersRaw = sessionStorage.getItem(prevFiltersKey);
        if (!prevFiltersRaw) {
            const currentFilters = restoreFilters(current.currentForm);
            if (currentFilters) {
                sessionStorage.setItem(prevFiltersKey, JSON.stringify(currentFilters));
            }
        }

        // navigate (page load will call restoreFormFilters on ready)
        if (prev.url) {
