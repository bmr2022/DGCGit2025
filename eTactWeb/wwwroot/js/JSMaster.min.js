...
/* =====================================================================
   DRILLDOWN MANAGER – FULL SYSTEM (STABLE VERSION)
   ===================================================================== */
const DrilldownManager = (() => {
    ...
    function goBack(root = "/") {
        ...
        restoreFormFilters(prev.nextForm);
    ...
    function restoreFormFilters(formName) {
        ...
        for (const id in filters) {
            ...
            if ($el.attr("type") === "checkbox") {
                ...
                $el.datepicker("setDate", filters[id]);
            }
            else {
                $el.val(filters[id]);
            }
    ...
    /* ---------------- CLEAR STACK IF FORM NOT FOUND ---------------- */
    function clearStackIfMissing(formName) {

        const stack = getStack();

        const exists = stack.some(item =>
            item.currentForm === formName ||
            item.nextForm === formName
        );

        if (!exists) {
            sessionStorage.removeItem(STACK_KEY);
            console.log(`🧹 Stack cleared (form "${formName}" not found in stack)`);
        }
        else {
    ...
    function toggleBackButton(selector = "#BackButton") {
        const stack = getStack();
        stack.length > 1 ? $(selector).show() : $(selector).hide();
    }
    ...
    return {
        ...
        goBack,
        toggleBackButton,
        clearStackIfMissing      // ⭐ ADDED HERE
    };
...
function collectFilters() {
    console.log("collectFilters called!");
    debugger
    const filters = {};
...
window.callDrillDown = function (elem) {
    ...
    const targetForm = $el.data("targetform") || "";

    // Save filters
    DrilldownManager.saveFormFilters(currentForm, collectFilters());
    ...
    // Save drilled values
    sessionStorage.setItem("DRILL__CLICK", $el.data("clickname") || "");
    sessionStorage.setItem("DRILL__TARGET_FORM", targetForm);
    sessionStorage.setItem("DRILL__GROUP", $el.data("groupname") || "");
    sessionStorage.setItem("DRILL__PARENT", $el.data("parentgroup") || "");
    sessionStorage.setItem("DRILL__ACCOUNT", $el.data("accountcode") || "");

    window.location.href = "/" + controller + "/" + action;
};

/* ============= RESTORE ============= */
function initDrillRestore(formName) {

    DrilldownManager.restoreFormFilters(formName);
    DrilldownManager.toggleBackButton();

    setTimeout(() => {
        let mode = sessionStorage.getItem("DRILL__TARGET_FORM");
        if (mode) $("#ReportType").val(mode).trigger("change");
        sessionStorage.removeItem("DRILL__TARGET_FORM");
    }, 200);
}
