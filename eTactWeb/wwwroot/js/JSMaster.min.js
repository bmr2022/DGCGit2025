...
/* ================================================================
   MULTI-LEVEL DRILLDOWN MANAGER (Final)
   Features:
   - Unlimited levels
   - Saves filters per page
   - Uses data-targetform to set ReportType on next page
   - Back button visibility (only when stack exists)
   - Safe auto-change flag so change-event does NOT run user logic
   - No controller changes required
================================================================ */

var isAutoChangingReportType = false; // GLOBAL FLAG used to skip manual change logic
...
function ddPush(level) {
    let stack = ddGetStack();
    stack.push(level);
    ddSaveStack(stack);
}
function ddPop() {
...
function ddSaveFilters(pageName, filters) {
    try {
        ...
        console.warn("ddSaveFilters error:", e);
    }
}
function ddLoadFilters(pageName) {
    try {
        let raw = sessionStorage.getItem(DD_FILTER_PREFIX + pageName);
        return raw ? JSON.parse(raw) : null;
    } catch (e) {
        console.warn("ddLoadFilters error:", e);
        return null;
    }
}

/* ---------------- MAIN DRILLDOWN (used by <a onclick="callDrillDown(...)"> ) ---------------- */
/*
  callDrillDown(el, pageName, filterObj)
   - el: link element (this)
   - pageName: string key to save filters under (eg "TrailBalance")
   - filterObj: object with current page filters (save to restore later)
*/
function callDrillDown(el, pageName, filterObj) {
    // default pageName if not provided
    pageName = pageName || "TrailBalance";
    filterObj = filterObj || {};

    // 1) Save filters for current page
    ddSaveFilters(pageName, filterObj);

    // 2) Read the target report type from data-targetform attribute (dynamic)
    var forceReportType = $(el).data("targetform") || "";

    // 3) Push stack entry
    ddPush({
        returnPage: window.location.href,
        returnPageName: pageName,
        forceReportType: forceReportType,
        drillData: $(el).data()
    });

    // 4) Build URL from data-controller / data-action / other data-*
    ...
    $.each($(el).data(), function (k, v) {
        if (k !== "controller" && k !== "action") {
            // note: jQuery lowercases data keys; we pass as-is
...
/* ---------------- PAGE INIT: shows/hides Back & applies forced ReportType ---------------- */
/*
  ddInitPage(backBtnId, reportDDLId)
   - backBtnId: id of Back button (string)
   - reportDDLId: id of ReportType dropdown (string)
*/
function ddInitPage(backBtnId, reportDDLId) {

    // 1) Back button show/hide
    if (backBtnId) {
        if (ddHasStack()) {
            $("#" + backBtnId).show();
        } else {
    ...
    // 2) Auto-set ReportType from top-of-stack if present
    var stack = ddGetStack();
    if (!stack || stack.length === 0) return;

    var last = stack[stack.length - 1];
    if (last && last.forceReportType && reportDDLId) {
        // set safely with flag so change handler can skip manual logic
        isAutoChangingReportType = true;
        $("#" + reportDDLId).val(last.forceReportType).trigger("change");
        // small timeout to ensure change handler ran (if it needs to inspect DOM)
        // then clear flag
        setTimeout(function () { isAutoChangingReportType = false; }, 0);
    }
}
