...
async function GetDbCrDetail() {
    ...
    if (DbCrPageData != null && DbCrPageData != undefined && DbCrPageData != '') {
        $.ajax({
            ...
            type: "POST",
            data: { "PageName": DbCrPageName, "docAccountCode": docAccountCode, "AccountCode": AccountCode, "BillAmt": BillAmt, "NetAmt": NetAmt, "RoundOffAccountCode": RoundOffAccountCode, "RoundOffAmount": RoundOffAmount },
            async: false,
...
const DrilldownManager = (() => {
    ...
    // --- 🔹 Clear Only the Stack (keep filters) ---
    function clearStack() {
        sessionStorage.removeItem(STACK_KEY);
        console.log("🧱 Drilldown navigation stack cleared.");
    }
    ...
    function clearAll() {
        ...
        for (let i = 0; i < sessionStorage.length; i++) {
            ...
            if (key && key.startsWith(FILTER_KEY)) {
                sessionStorage.removeItem(key);
            }
        }
        console.log("🧹 Drilldown stack and filters cleared manually.");
    }

    // --- 🔹 Clear stack only if it does NOT contain a given form ---
    function clearStackIfMissing(formName) {

        const stack = getStack() || [];

        const exists = stack.some(item =>
            item.currentForm === formName || item.nextForm === formName
        );

        if (!exists) {
            sessionStorage.removeItem(STACK_KEY);
            console.log(`🧱 Stack cleared because "${formName}" was not found.`);
        } else {
            console.log(`✅ Stack preserved (contains "${formName}").`);
        }
    }
    ...
    function goBack(rootUrl = "/") {
        ...
        const prevForm = prev.nextForm || null;
...
// ======================================================
// 🧹 Auto-Clear Drilldown Stack & Filters on Page Refresh (F5)
// ======================================================
$(window).on("load", function () {

    // Detect if the page load is triggered by a reload (F5, Ctrl+R, browser reload)
    const navEntries = performance.getEntriesByType("navigation");
    const isReload =
        (performance.navigation.type === performance.navigation.TYPE_RELOAD) ||
        (navEntries && navEntries.length && navEntries[0].type === "reload");

    if (isReload) {
        console.log("🔄 Page reloaded → clearing drilldown stack & filters");

        // 🧱 Clear the navigation stack
        sessionStorage.removeItem("DRILL_STACK");

        // 🧹 Clear all saved filters
        for (let i = 0; i < sessionStorage.length; i++) {
            const key = sessionStorage.key(i);
            if (key && key.startsWith("DRILL_FILTER_")) {
                sessionStorage.removeItem(key);
            }
        }

        console.log("✅ Drilldown stack and filters cleared due to reload.");
    }
});


// ======================================================
// ====== PART: Common Drill Utilities (for JSMaster.js) ======
// Uses sessionStorage to pass transient drill values (no controller changes)
// ======================================================
(function (window, $) {
    'use strict';

    // namespaced session keys
    const SKEY_PREFIX = "DRILL__";
    const SKEY = {
        TARGET: SKEY_PREFIX + "TARGET_FORM",
        CLICK: SKEY_PREFIX + "CLICKNAME",
        GROUP: SKEY_PREFIX + "GROUP",
        PARENT: SKEY_PREFIX + "PARENT",
        ACCOUNT: SKEY_PREFIX + "ACCOUNT",
        EXTRA: SKEY_PREFIX + "EXTRA"
    };

    // Save all current form fields (inputs/selects that have an id) into DrilldownManager
    function saveCurrentFormFilters(formName) {
        if (!formName) return;
        const filters = {};
        $("[id]").each(function () {
            ...
            if ($(this).attr("type") === "checkbox") {
                filters[id] = $(this).is(":checked");
            } else {
                filters[id] = $(this).val();
            }
        });

        if (window.DrilldownManager && typeof DrilldownManager.saveFormFilters === "function") {
            DrilldownManager.saveFormFilters(formName, filters);
        } else {
            console.warn("DrilldownManager.saveFormFilters not found.");
        }
    }

    // Generic click handler to call from markup: onclick="callDrillDown(this)"
    window.callDrillDown = function (elem) {
        const $el = $(elem);
        if (!$el.length) return;
        ...
        if (!controller || !action) {
            console.warn("callDrillDown: missing data-controller or data-action.");
            return;
        }

        const clickname = $el.data("clickname") || "";
        const targetForm = $el.data("targetform") || "TransactionLedger";

        const group = $el.data("groupname") || "";
        const parent = $el.data("parentgroup") || "";
        const account = $el.data("accountcode") || "";
        const extra = $el.data("extra") || null;

        // current form name detection
        const currentForm = ($("#FormName").val() || $("#ReportType").val() || document.title || "COMMON_FORM").toString();

        // save current filters
        try { saveCurrentFormFilters(currentForm); } catch (e) { console.error(e); }

        // push navigation
        if (window.DrilldownManager && typeof DrilldownManager.pushNavigation === "function") {
            DrilldownManager.pushNavigation({
        ...
        // save transient drill values in sessionStorage
        try {
            sessionStorage.setItem(SKEY.TARGET, targetForm);
            sessionStorage.setItem(SKEY.CLICK, clickname);
            sessionStorage.setItem(SKEY.GROUP, group);
            sessionStorage.setItem(SKEY.PARENT, parent);
            sessionStorage.setItem(SKEY.ACCOUNT, account);
            if (extra !== null && extra !== undefined) {
                sessionStorage.setItem(SKEY.EXTRA, (typeof extra === "string") ? extra : JSON.stringify(extra));
            }
        } catch (e) {
            console.warn("sessionStorage set failed:", e);
        }

        // optionally save minimal defaults for target form
        if (window.DrilldownManager && typeof DrilldownManager.saveFormFilters === "function") {
            const basic = {
            ...
            DrilldownManager.saveFormFilters(targetForm, basic);
        }

        // redirect to controller/action
        const destination = `/${controller}/${action}`;
        window.location.href = destination;
    };

    // Generic initializer: call on target pages to restore drill values
    // options: { formName: "TransactionLedger", map: { "DRILL__ACCOUNT": ["LedgerName","AccountCode"] }, clearAfterRestore: true, waitForSelect2Timeout: 4000 }
    window.initDrillRestore = function (options) {
        if (!options || !options.formName) return;
        const formName = options.formName;
        const map = options.map || {};
        const clearAfter = (typeof options.clearAfterRestore === "undefined") ? true : !!options.clearAfterRestore;
        const maxWait = options.waitForSelect2Timeout || 4000;

        const values = {
            "DRILL__ACCOUNT": sessionStorage.getItem(SKEY.ACCOUNT),
            "DRILL__GROUP": sessionStorage.getItem(SKEY.GROUP),
            "DRILL__PARENT": sessionStorage.getItem(SKEY.PARENT),
            "DRILL__CLICKNAME": sessionStorage.getItem(SKEY.CLICK),
            "DRILL__EXTRA": sessionStorage.getItem(SKEY.EXTRA)
        };

        // wait helper for selects / select2
        function waitForOptions(id, cb) {
            const start = Date.now();
            const iv = setInterval(function () {
                if ($(`#${id}`).length > 0 && $(`#${id} option`).length > 0) {
                    clearInterval(iv);
                    cb(true);
        ...
        // apply mapping
        Object.keys(map).forEach(function (drillKey) {
            const elIds = Array.isArray(map[drillKey]) ? map[drillKey] : [map[drillKey]];
            const val = values[drillKey];
            if (!val) return;
            ...
            elIds.forEach(function (id) {
                ...
                if ($el.is("select") || $el.hasClass("select2") || $el.hasClass("custom-select2")) {
                    waitForOptions(id, function () {
                        try { $el.val(val).trigger("change"); } catch (e) { }
                    });
                } else {
                    try { $el.val(val).trigger("change"); } catch (e) { }
                }
        ...
        // restore saved filters for this form
        if (window.DrilldownManager && typeof DrilldownManager.restoreFormFilters === "function") {
            try { DrilldownManager.restoreFormFilters(formName); } catch (e) { console.error(e); }
        }

        // clear transient session keys if required
        if (clearAfter) {
            try {
                Object.values(SKEY).forEach(k => sessionStorage.removeItem(k));
            } catch (e) { console.warn("Failed clearing drill session keys:", e); }
        }

        // toggle back button
        setTimeout(() => {
            if (window.DrilldownManager && typeof DrilldownManager.toggleBackButton === "function") {
                DrilldownManager.toggleBackButton();
            }
        }, 150);
    };
