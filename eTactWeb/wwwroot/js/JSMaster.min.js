$(document).ready(function () {
    ...
    $('title').text().indexOf('SaleInvoice') == 0 ? $('#AdjDrCr').val("DR") : $('#AdjDrCr').val("CR");
...
function parseDateToFormat2(inputDate) {
    inputDate = inputDate.trim();

    // Extract date part (before the time)
    const datePart = inputDate.split(' ')[0]; // "5/25/2025"
...
function parseDateToFormat(inputDate) {
    ...
    const dateFormats = [
        { regex: /^(\d{1,2})\/(\d{1,2})\/(\d{4}) \d{1,2}:\d{2}:\d{2} (AM|PM)$/i, type: 'mdy' },
        // MM/dd/yyyy without time
        { regex: /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/, type: 'mdy' },
        { regex: /\b(\d{1,2})[-/](\d{1,2})[-/](\d{4})\b/, type: 'dmyOrMdy' },
        { regex: /\b(\d{4})[-/](\d{1,2})[-/](\d{1,2})\b/, type: 'ymd' },
        { regex: /\b(\d{1,2})[/](\w{3})[/](\d{4})\b/, type: 'dMonY' },
        { regex: /\b(\d{1,2})[\/](\d{1,2})[\/](\d{2})\b/, type: 'dmyShort' },
        { regex: /\b(\d{1,2})-([A-Za-z]{3})-(\d{2})\b/, type: 'dMonYShort' },
        { regex: /\b(\d{1,2})-([A-Za-z]{3})-(\d{4})\b/, type: 'dMonYFull' },
        { regex: /\b(\d{1,2})[-/](\d{1,2})[-/](\d{4}) (\d{1,2}):(\d{2}):(\d{2}) ?(AM|PM|am|pm)?\b/, type: 'datetime' },
        { regex: /\b(\d{1,2})\/([A-Za-z]{3})\/(\d{4})\b/i, type: 'dMonYFull' },
        { regex: /\b(\d{1,2})\/(\d{1,2})\/(\d{4})\b/, type: 'mdy' },
        { regex: /\b(\d{1,2})\/(\d{1,2})\/(\d{4}) (\d{1,2}):(\d{2}):(\d{2}) (AM|PM|am|pm)\b/, type: 'datetimeMdy' }
...
function AddAdjstmntDetail1() {
    ...
    if (result.toString().toLowerCase() == "false" && modeofadjst != null && modeofadjst != "AgainstRef") {
        AddAdjstmntDetail();
    }
}
...
function Agnstrefonsave() {
    ...
    if (parseFloat(adjustedamtsum.toFixed(2)) > parseFloat(comparedamt.toFixed(2)) || parseFloat(adjustedamt.toFixed(2)) > parseFloat(billamtsum)) {
