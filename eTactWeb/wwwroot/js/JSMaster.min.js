...
// wwwroot/js/drilldownManager.js
var DrilldownManager = (function () {
    const stackKey = "DrilldownStack";
    const filterPrefix = "Filters_";

    function getStack() {
        let s = sessionStorage.getItem(stackKey);
    ...
    function push(formName, url, filters) {
        let stack = getStack();
        stack.push({ form: formName, url, filters });
        saveStack(stack);
        sessionStorage.setItem(filterPrefix + formName, JSON.stringify(filters || {}));
    }
    ...
    function restoreFilters(formName) {
        let s = sessionStorage.getItem(filterPrefix + formName);
        return s ? JSON.parse(s) : null;
    }

    function navigateTo(nextFormName, nextUrl, currentFormName) {
        let filters = collectFilters();
        push(currentFormName, window.location.pathname + window.location.search, filters);
...
// -------------------------------
// 🔹 DrillDown Navigation (on click)
// -------------------------------
window.callDrillDown = function (elem) {
    var $a = $(elem);

    var controller = $a.data('controller');
    var action = $a.data('action');
    var docid = $a.data('docid');
    var accountcode = $a.data('accountcode');
    var yearcode = $a.data('yearcode');
    ...
    params.append('Mode', 'U');

    var nextUrl = `/${controller}/${action}?${params.toString()}`;

    // ✅ Use DrilldownManager to store current filters + navigate
    var currentForm = $('#headerId').text().trim().replace(/\s+/g, '');
    DrilldownManager.navigateTo(action, nextUrl, currentForm);
};
