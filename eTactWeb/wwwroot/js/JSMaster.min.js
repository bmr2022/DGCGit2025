...
function parseDateToFormat(inputDate) {
    //const dateFormats = [
    //    ///\b\w{3} (\w{3}) (\d{2}) (\d{4})\b/,      // Wed Apr 24 2024
    //    /\b(\d{2})[-/](\d{2})[-/](\d{4})\b/,     // dd/MM/yyyy or dd-MM-yyyy
    //    /\b(\d{4})[-/](\d{2})[-/](\d{2})\b/,     // yyyy/MM/dd or yyyy-MM-dd
    //    /\b(\d{2})[/](\w{3})[/](\d{4})\b/,        // dd/Mon/yyyy
    //    /\b(\d{2})[\/](\d{2})[\/](\d{2})\b/,      // MM/dd/yy
    //    // Add your new format regular expression here
    //];
    ...
    for (const format of dateFormats) {
        const match = inputDate.match(format);
        console.log("Trying Format:", format, "Match:", match); // Debug match result

        if (match) {
            if (format === dateFormats[0] && parseInt(match[1]) > 12) {
                [, day, month, year] = match; // dd/MM/yyyy
            } else if (format === dateFormats[0]) {
                [, month, day, year] = match; // MM/dd/yyyy
            } else if (format === dateFormats[1]) {
                [, year, month, day] = match; // yyyy/MM/dd
            } else if (format === dateFormats[2]) {
                ...
                [, month, day, year] = match; // MM/dd/yy
                year = `20${year}`; // Convert 2-digit year to 4 digits
            } else if (format === dateFormats[4]) {
                [, day, month, year] = match; // dd-MMM-yy
                year = `20${year}`; // Convert 2-digit year to 4 digits
            } else if (format === dateFormats[5]) {
    ...
    $.ajax({
        ...
        success: function (data, status, error) {
            ...
            commonElements.forEach(function (element) {
                element = element.replaceAll(" ", "");
                var optSave = obj.Result[0].OptSave;
                var optView = obj.Result[0].OptView;
                if (optSave == true) {
            ...
            //console.log(notCommonElementsjQuery);
        },
        ...
        if ($('#TxType').val() == "TAX") {
            ...
            $.ajax({
                ...
                success: function (data, status, xhr) {
                    ...
                    if ($('title').text().indexOf('Direct Purchase Bill') == 0 || ($('title').text().indexOf('Purchase Bill') == 0 && $('title').text().indexOf('Direct') != 0) || $('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('Credit Note ') == 0) {
                        GetDbCrDetail();
                    }
                },
                error: function (error, status, xhr) {
                        ...
                        title.indexOf('SaleInvoice') === 0 ? 'SaleInvoice' :
                            title.indexOf('Purchase Order') === 0 ? 'PurchaseOrder' :
                                title.indexOf('Credit Note') === 0 ? 'CreditNote' :
                                    title.indexOf('Credit Note') === 0 ? 'PurchaseRejection' :
                                    'Unknown';
            console.log("seqNo: " + SeqNo + ", " + SN);
...
async function ApplyTax4HSN() {
    ...
    if ($('#TxType').val() == 'TAX' && $('#TxPartCode').prop('selectedIndex') == 0 && $('#TxItemCode').prop('selectedIndex') == 0) {
        ...
        if (objData.indexOf('DeleteTaxRow(1)') !== -1) {
            ...
            if ($('title').text().indexOf('Direct Purchase Bill') == 0 || ($('title').text().indexOf('Purchase Bill') == 0 && $('title').text().indexOf('Direct') != 0) || $('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('Sale Rejection') == 0 || $('title').text().indexOf('Credit Note') == 0) {
                GetDbCrDetail();
            }
        }
...
function AddTDSDetail() {
    ...
    $.ajax({
        ...
        success: function (resp, status, error) {
            ...
            if (TxPageData == 'DirectPurchaseBill' || TxPageData == 'PurchaseBill' || TxPageData == 'SaleInvoice' || TxPageData == 'SaleRejection' || TxPageData == 'CreditNote') {
...
function AddAdjstmntDetail1() {
    ...
    else if (result.toString().toLowerCase() == "false" && modeofadjst != null && modeofadjst != "AgainstRef") {
        AddAdjstmntDetail();
    }
}
