    ...
    $.ajax({
        ...
        success: function (resp, status, error) {
            if (resp.trim() == "") {
                ...
                $('#NetTotal').val(NT.toFixed(6));
                $('#BaseNetTotal').val(NT.toFixed(6));
                $('#TxType').prop('selectedIndex', 0).change();
                $('#AdjPendAmt').val(parseFloat($('#NetTotal').val() || 0).toFixed(6));
            }
        ...
        if ($('#TxType').val() == "TAX") {
            ...
            $.ajax({
                ...
                success: function (data, status, xhr) {
                    ...
                    if ($('title').text().indexOf('Direct Purchase Bill') == 0 || $('title').text().indexOf('Purchase Rejection') == 0 || ($('title').text().indexOf('Purchase Bill') == 0 && $('title').text().indexOf('Direct') != 0) || $('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('Credit Note ') == 0) {
                        GetDbCrDetail();
                        //AddAdjstmntDetail1();
                    }
...
//            dataType: "json",
//            success: function (data, status, error) {
//                $('#TxAmount').val(data.amt.toFixed(6)), $('#TxOnExp').val(data.taxOnExp.toFixed(6));
//            },
//            error: function (req, status, error) {
//                $.notify(error, { position: "top center", className: "error" });
//            }
...
$('.btnClearTax').click(function (e) {
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            ...
            if ($('title').text().indexOf('Direct Purchase Bill') == 0 || $('title').text().indexOf('Purchase Rejection') == 0 || ($('title').text().indexOf('Purchase Bill') == 0 && $('title').text().indexOf('Direct') != 0) || $('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('Purchase Rejection') == 0) {
                GetDbCrDetail();
            }
        }
    }).done(function (data) {
        ...
        if ($('#TxPartCode').prop('selectedIndex') != 0 && $('#TxTaxType').prop('selectedIndex') != 0 && $('#TxAccountCode').prop('selectedIndex') != 0) {
            $.ajax({
                ...
                success: function (data, status, error) {
                    ...
                    $('#TxAmount').val(data.amt.toFixed(6));
                    $('#TxOnExp').val(data.taxOnExp.toFixed(6));
                    ROF();
                },
                error: function (req, status, error) {
                    $.notify(error, { position: "top center", className: "error" });
    ...
    if ($.isNumeric($('#NetTotal').val()) && parseFloat($('#NetTotal').val()) > 0) {
        if ($(this).val() == "Y") {
            ...
            if (TxPageData == 'DirectPurchaseBill' || TxPageData == 'PurchaseBill') {
                ...
                $('#TotalRoundOffAmt').val(RoundOffAmt.toFixed(6));
            }
            Amt = Math.round(Amt);
            $('#NetTotal').val(Amt.toFixed(6));
            $('#BaseNetTotal').val(Amt.toFixed(6));
...
function TDP() {
    ...
    if (DP !== 0) {
        ...
        $('#TotalAmtAftrDiscount').val(DA.toFixed(6));
        Amount = Amount - DA;
    }
    else {
    ...
    $('#NetTotal').val(Amount.toFixed(6));
    $('#BaseNetTotal').val(Amount.toFixed(6));
    $('#AdjPendAmt').val(parseFloat($('#NetTotal').val() || 0).toFixed(6));
}
...
async function DeleteTaxRow(SeqNo) {
        ...
        if ($('#txGridName-' + SeqNo).text().includes("CGST")) {
            ...
            $.ajax({
                ...
                success: await function (data, status, xhr) {
                    ...
                    $('#NetTotall').val(sumTax.toFixed(6));
                    $('#BaseNetTotal').val(sumTax.toFixed(6));
    ...
    if (!$('#txGridName-' + SeqNo).text().includes("GST") && $('#txGridName-' + SeqNo).text() != "IGST" && !$('#txGridName-' + SeqNo).text().includes("CGST") && !$('#txGridName-' + SeqNo).text().includes("SGST")) {
        var title = $('title').text();
        var SN = title.indexOf('Sale Order') === 0 ? 'ItemList' :
            title.indexOf('Direct Purchase Bill') === 0 ? 'DirectPurchaseBill' :
                (title.indexOf('Purchase Bill') === 0 && title.indexOf('Direct') !== 0) ? 'PurchaseBill' :
...
async function ApplyTax4HSN() {
    ...
    if ($('#TxType').val() == 'TAX' && $('#TxPartCode').prop('selectedIndex') == 0 && $('#TxItemCode').prop('selectedIndex') == 0) {
        ...
        if (objData.indexOf('DeleteTaxRow(1)') !== -1) {
            ...
            if ($('title').text().indexOf('Direct Purchase Bill') == 0 || $('title').text().indexOf('Purchase Rejection') == 0 || ($('title').text().indexOf('Purchase Bill') == 0 && $('title').text().indexOf('Direct') != 0) || $('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('Sale Rejection') == 0 || $('title').text().indexOf('Credit Note') == 0) {
                GetDbCrDetail();
                AddAdjstmntDetail1();
            }
        }
...
function CalculateNetAmountWithTDS() {
    ...
    $('#NetTotal').val(TotalSum.toFixed(6));
    $('#BaseNetTotal').val(TotalSum.toFixed(6));
    $('#AdjPendAmt').val(parseFloat($('#NetTotal').val() || 0).toFixed(6));
        ...
        if (/*$('#TxPartCode').prop('selectedIndex') != 0 && */$('#TDSTaxType').prop('selectedIndex') != 0 && $('#TDSAccountCode').prop('selectedIndex') != 0) {
            $.ajax({
                ...
                success: function (data, status, error) {
                    ...
                    $('#TDSAmount').val(data.amt.toFixed(6));
                    let RoundOffAmt = 0;
                    RoundOffAmt = Math.abs(data.amt.toFixed(6) - Math.round(data.amt.toFixed(6)));
                    $('#TDSRoundOffAmt').val(RoundOffAmt.toFixed(6));
                    TDSROF();
                },
                error: function (req, status, error) {
                    $.notify(error, { position: "top center", className: "error" });
        ...
        if ($('#tdsGridName-' + SeqNo).text().includes("CGST")) {
            ...
            $.ajax({
                ...
                success: await function (data, status, xhr) {
                    ...
                    //$('#NetTotall').val(sumTds.toFixed(6));
            ...
            success: function (result, status, xhr) {
                ...
                if (data.length === 0) {
                    ...
                    $.each(data.Result, function (index, item) {
                        ...
                        rows += '<td class="dramt">' + parseFloat(debitAmount).toFixed(6) + '</td>';
                        rows += '<td class="cramt">' + parseFloat(creditAmount).toFixed(6) + '</td>';
                        rows += '</tr>';
                        totalDr += debitAmount;
                    ...
                    $('#TotalDr').text(totalDr.toFixed(6));
                    $('#TotalCr').text(totalCr.toFixed(6));
                }
            },
...
function AddAdjstmntDetail() {
    ...
    var AdjustmentData = {
        ...
        "AdjDueDate": parseDateToFormat($('#AdjDueDate').val()),
        "DueDate": parseDateToFormat( $('#AdjDueDate').val()),
        "AdjNewRefNo": $('#AdjNewRefNo').val(),
        "AdjPendAmt": parseFloat($('#AdjPendAmt').val() || 0).toFixed(6),
...
function CalculateAdjAmt(netamtid) {
    ...
    // ✅ only apply .toFixed(6) at the end when displaying
    $('#AdjPendAmt').val(resultpendamt.toFixed(6));
    $('#AdjRemainingAmt').val(resultpendamt.toFixed(6));
    $('#AdjTotalAmt').val(netamt.toFixed(6));
    $('#AdjAdjstedAmt').val(sum.toFixed(6));

    // Update the footer values
    $('#AdjTotalAmt1').text(netamt.toFixed(6));
    $('#AdjAdjstedAmt1').text(sum.toFixed(6));
    $('#AdjRemainingAmt1').text(resultpendamt.toFixed(6));
}
...
function CheckAndValidateAdjAmtAtTimeofAdd(netamtid, pendAmt) {
    ...
    var amtforadd = pendAmt + sum;
    amtforadd = parseFloat(amtforadd.toFixed(6));
            ...
            if (data === "Duplicate" && (xhr.statusCode === 207 || xhr.responseText === "Duplicate")) {
                ...
                if (isEditAllowed == "false") {
                    ...
                    if (obj != null && obj.length == 1) {
                        ...
                        $('#AdjDueDate').val(obj[0].AdjDueDate);
        ...
        $.ajax({
            ...
            success: function (result, status, error) {
                if (result != null) {
                    ...
                    $('#TotalBillAmount').val(totals.billTotal);
                    $('#TotalRemainingAmount').val(totals.remainingTotal)
...
function CalculateTotals() {
    ...
    // Bill (Adjusted Amount)
    $('#divDisplayAdjustmntDetail input[id^="AdjAgnstPendAmt_"]').each(function () {
...
function ChangeAdjustAmount() {
    debugger
    var rowCount = $('#divDisplayAdjustmntDetail tr').length - 1; // 0-based index
    var totalAdjustStr = $('#TotalAdjustAmount').val();

    // Step 1: Validation
    if (totalAdjustStr.includes('-')) return;
    var totalAdjust = parseFloat(totalAdjustStr);
    if (isNaN(totalAdjust) || totalAdjust <= 0) return;

    // Step 2: Store original balances (once)
    ...
    // Step 3: Reset all rows
    for (let i = 0; i <= rowCount; i++) {
        const $bal = $('#AdjAgnstPendAmt_' + i);
        const $adjAmt = $('#TotalAdjustAmount-' + i);
    ...
    // Step 4: Distribute Total Adjust Amount row-wise
    ...
    for (let i = 0; i <= rowCount; i++) {
        ...
        // If-else allocation (no Math.min)
        let adjustHere;
        if (remainingQty >= original) {
            adjustHere = original;
        } else {
            adjustHere = remainingQty;
        }
    ...
    // Step 5: Unchecked rows reset background
    for (let i = 0; i <= rowCount; i++) {
        if (!$('#AgnstRefCheckbox-' + i).is(':checked')) {
            $('#DisplayAdjustmntRow-' + i).css("background-color", "");
        }
    }
}
function ChangeAdjustAmt(i) {
    ...
    let balanceAmtLabel = $('#AdjAgnstPendAmt_' + i); // Total balance per row
    let remainingAmtInput = $('#AdjAgnstRemainingAmt_' + i);   // Remaining after adjustment
    let errorHeader = $('#AdjustmentHeaderError');

    let originalBalance = parseFloat(balanceAmtLabel.attr("data-original")) || 0;
...
function Agnstrefonsave() {
    ...
    if (AdjPageName.indexOf('Sale Order') === 0) {
        ...
        AdjPageData = 'DirectPurchaseBill';
        PaymentDays = ($('#PaymentDays').val() != null && $('#PaymentDays').val() != '' && $('#PaymentDays').val() != undefined) ? parseInt($('#PaymentDays').val()) : 0;
    }
    else if (AdjPageName.indexOf('Purchase Bill') == 0) {
        AdjPageData = 'PurchaseBill';
        PaymentDays = ($('#PaymentDays').val() != null && $('#PaymentDays').val() != '' && $('#PaymentDays').val() != undefined) ? parseInt($('#PaymentDays').val()) : 0;
    ...
    var selectedItems1 = [];
    var IsErrorOnAddAgnst = false;
    var errorstring = '';
    var adjustedamt = 0;
    var adjustedamtsum = 0;
    var billamtsum = 0;
    var nettotal = ($('#AdjTotalAmt1').text() != '' && $('#AdjTotalAmt1').text() != undefined && $('#AdjTotalAmt1').text() != 0) ? parseFloat($('#AdjTotalAmt1').text()) : 0;
    var adjamt1 = ($('#AdjAdjstedAmt1').text() != '' && $('#AdjAdjstedAmt1').text() != undefined && $('#AdjAdjstedAmt1').text() != 0) ? parseFloat($('#AdjAdjstedAmt1').text()) : 0;
    var netamt = ($('#NetTotal').val() != '' && $('#NetTotal').val() != undefined && $('#NetTotal').val() != 0) ? parseFloat($('#NetTotal').val()) : 0;
    var comparedamt = (nettotal > 0) ? nettotal : netamt;
    adjustedamtsum = adjustedamtsum + adjamt1;
    ...
    $('tr[id^="DisplayAdjustmntRow-"]').each(function () {
        ...
        if ($('#AgnstRefCheckbox_' + rowIndex).is(':checked')) {
            ...
            var formattedDate = rawDate ? addDaysToDate(rawDate, PaymentDays) : null; // Only add days if date is not null
            var pendamt = parseFloat($('#AdjAgnstPendAmt_' + rowIndex).val());
            var rmngamt = parseFloat($('#AdjAgnstRemainingAmt_' + rowIndex).val());
            adjustedamt = parseFloat($('#AdjAgnstAdjstedAmt_' + rowIndex).val());
            var item = {
                ...
                AdjAgnstVouchDate: formattedDate, // Format date
                AdjAgnstModeOfAdjstment: $('#AdjAgnstModeOfAdjstment_' + rowIndex).val(),
                AdjAgnstVouchType: $('#AdjAgnstVouchType_' + rowIndex).val(),
                AdjAgnstPendAmt: pendamt,
                AdjAgnstRemainingAmt: rmngamt,
                AdjAgnstAdjstedAmt: adjustedamt,
                AdjAgnstDrCr: $('#AdjAgnstDrCr_' + rowIndex).val(),
                AdjPageName: AdjPageName
            };
            adjustedamt = (adjustedamt != null && adjustedamt != undefined && !isNaN(adjustedamt)) ? adjustedamt : 0;
            adjustedamtsum += adjustedamt;
            pendamt = (pendamt != null && pendamt != undefined && !isNaN(pendamt)) ? pendamt : 0;
            billamtsum += pendamt;
    ...
    if (parseFloat(adjustedamtsum.toFixed(6)) > parseFloat(comparedamt.toFixed(6)) || parseFloat(adjustedamt.toFixed(6)) > parseFloat(billamtsum)) {
        errorstring = "TOTAL ADJUSTED AMT CAN NOT BE GREATET THEN NET AMT";
        IsErrorOnAddAgnst = true;
    }
    // Serialize model to JSON
    var jsonString1 = JSON.stringify(model);
    if (IsErrorOnAddAgnst != true) {
        $.ajax({
            type: "POST",
            ...
            dataType: 'json',
            success: function (resp, status, error) {
                console.log('Save Agnst ref popup successful', resp);
            ...
            error: function (error) {
                console.log('Save Agnst ref popup failed', error);
            }
        });
    }
    else {
        $.notify(errorstring, { position: "top center", className: "error" });
    }
}
...
function updateAdjGridTable(data) {
    ...
    data.forEach(function (item) {
        ...
        var row = `<tr class="small ${item.adjSeqNo}" style="white-space:nowrap">
            ...
            <td id="GridAdjPendAmount-${item.adjSeqNo}">${item.adjPendAmt ? item.adjPendAmt.toFixed(6) : "0.00"}</td>
...
function ValidateAdjAgnstRefPopupOnSave(index) {
    ...
    if (parseFloat(adjamt.toFixed(6)) > parseFloat(billamt.toFixed(6))) {
        iserror = true;
...
function onAgnstcheckcliclunclick(index) {
    ...
    if ($('#AgnstRefCheckbox_' + index).is(':checked')) {
        if (adjamt > 0 && adjamt < billamt) {
            $('#AdjAgnstAdjstedAmt_' + index).val(adjamt);
        }
        else {
            $('#AdjAgnstAdjstedAmt_' + index).val(billamt);
        }
    }
    else {
...
async function DeleteTaxRowByItemCode(itemCode) {
        ...
        title.indexOf('Direct Purchase Bill') === 0 ? 'DirectPurchaseBill' :
            title.indexOf('Purchase Bill') === 0 && title.indexOf('Direct') !== 0 ? 'PurchaseBill' :
                title.indexOf('Purchase Rejection') === 0 ? 'PurchaseRejection' :
                    title.indexOf('SaleInvoice') === 0 ? 'SaleInvoice' :
                        title.indexOf('Purchase Order') === 0 ? 'PurchaseOrder' :
                            title.indexOf('Credit Note') === 0 ? 'CreditNote' : 'Unknown';
    ...
    try {
        ...
        $('#TotalTaxAmt').val(txSum.toFixed(6));

        const INA = parseFloat($('#ItemNetAmount').val()) || 0;
        const TTA = parseFloat($('#TotalTaxAmt').val()) || 0;
        $('#NetTotal').val((INA + TTA).toFixed(6));
        $('#BaseNetTotal').val((INA + TTA).toFixed(6));
        ...
        if (['Direct Purchase Bill', 'Purchase Rejection', 'Purchase Bill', 'SaleInvoice', 'Credit Note'].some(str => title.includes(str))) {
            GetDbCrDetail();
        }
