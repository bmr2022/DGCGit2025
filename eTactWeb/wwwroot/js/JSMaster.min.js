//add by kinjal 17/03/2026 for int/decimal validation of qty input based on unit master setup
function validateQtyByUnit(unit, qtyInputId) {
    ...
    $.ajax({
        ...
        url: '/Common/CheckRoundOff',
        data: { unit: unit },
        dataType: "json",
        async: false,
        success: function (response) {
            var obj = $.parseJSON(response);
            let $input = $('#' + qtyInputId);

            if ($('title').text().indexOf('Sale Rejection') == 0) {
            ...
            if (obj.Result[0].Round_Off === "Y" || obj.Result.Table[0].Round_Off === "y") {
                // ONLY INTEGER (block decimal)
                $input.on('input', function () {
                    this.value = this.value.replace(/[^0-9]/g, '');
                });
            }
            // else → DO NOTHING (default already allows decimal)
        }
    });
}
document.addEventListener("click", function (e) {

    let link = e.target.closest("a[href*='preview.printReport']");
...
function allowPositiveNumberJQ(selector) {
...
function allowNumber(selector, decimalPlaces, allowNegative = false) {
    ...
    $(document).on('keypress', selector, function (e) {
        ...
        let isFullSelected = selectionStart === 0 && selectionEnd === value.length;

        // allow backspace, tab, arrows
        if (charCode === 8 || charCode === 9 || charCode === 37 || charCode === 39)
            return true;

        // allow minus only if negative allowed
        if (allowNegative && charCode === 45 && value.indexOf('-') === -1 && selectionStart === 0)
            return true;

        // allow decimal point
        if (charCode === 46 && (value.indexOf('.') === -1 || isFullSelected))
            return true;

        // allow numbers
        if (charCode >= 48 && charCode <= 57) {

            // if full text selected → allow replace
            if (isFullSelected)
                return true;

            if (value.indexOf('.') !== -1) {
                let decimalPart = value.split('.')[1];
                if (decimalPart && decimalPart.length >= decimalPlaces && selectionStart > value.indexOf('.'))
                    return false;
            }
...
function FillReportType(TableName, MainReportType) {
    $.ajax({
        type: "POST",
        url: '/Common/FillReportTypes',
        dataType: "JSON",
        async: false,
        data: { "TableName": TableName, "MainReportType": MainReportType },
        success: function (result, status, error) {
            if (result != null) {
                var obj = $.parseJSON(result);
                if (obj.StatusCode == 200) {
                    $('#ReportType').empty();
                    $('#ReportType').append('<option value="0" selected>-Select-</option>');
                    $.each(obj.Result, function (index, val) {
                        $('#ReportType').append('<option selected value="' + val.ReportType +
...
function hideEmptyParentLis() {

    let changed = false;

    // 🔒 STRICTLY limit to menu containers
    $('#main_nav li.menu-parent').each(function () {

        var $parentLi = $(this);

        // must have submenu
        var $childLis = $parentLi.children('ul').children('li');
        ...
        if (!anyVisible && $parentLi.css('display') !== 'none') {
            $parentLi.hide();
            changed = true;
        }
    });

    // 🔁 recursively hide upper parents (QCMASTER → ADMIN → etc.)
    if (changed) {
        hideEmptyParentLis();
    }
}
...
function CheckPermissionForHeaders() {
    ...
    $.ajax({
        ...
        success: function (data, status, error) {
            ...
            obj.Result.forEach(row => {
                var parent = row.ParentMenu || row.parentmenu;

                if (!parent || !row.MainMenu) return;

                var parentId = parent.replaceAll(" ", "");
                var childHideId = row.MainMenu.replaceAll(" ", "") + 'Hide';
                ...
                parentChildMap[parentId].push({
                    hideId: childHideId,
                    isVisible: row.IsVisible   // 🔑 from SP
                });
            });
            ...
            Object.keys(parentChildMap).forEach(parentId => {
                ...
                var allInvisibleInSP = children.every(c => c.isVisible === 'N');

                if (!allInvisibleInSP) return; // ⛔ do NOT hide parent

                // optional extra safety: check DOM state also
                var allHiddenInDOM = children.every(c => {
                    return $('#' + c.hideId).length > 0 && $('#' + c.hideId).is(':hidden');
                });
            ...
            notCommonElementsjQuery.forEach(function (element) {
                ...
                $('#' + element).removeAttr("style").css({ "display": "none" });
                $('#' + element + '1').removeAttr("style").css({ "display": "none" });
                $('#' + element + 'Hide').removeAttr("style").css({ "display": "none" });
                $('#' + element + 'Divider').removeAttr("style").css({ "display": "none" });
            ...
            obj.Result.forEach(row => {
                ...
                if (optSave) {
                    ...
                    $('#' + element + '1').removeAttr("style").css({
                        "pointer-events": "none",
                        "color": "gray",
                        "background-color": "#d3d3d3"
                    });
...
function ClearTax() {
    $('#TxPartCode').prop('selectedIndex', 0).prop('disabled', false).removeClass('is-invalid').change(),
        $('#TxItemCode').prop('selectedIndex', 0).prop('disabled', false).removeClass('is-invalid'),

        $('#TxTaxType').prop('selectedIndex', 0).removeClass('is-invalid'),
        $('#TxAccountCode').prop('selectedIndex', 0).removeClass('is-invalid'),
        $('#TxPercentg').val(0).removeAttr('readonly').removeClass('is-invalid'),
        $('#TxAdInTxable').prop('selectedIndex', 0).prop('disabled', false).removeClass('is-invalid'),
        $('#TxRoundOff').prop('selectedIndex', 0).removeClass('is-invalid'),
        $('#TxRefundable').prop('selectedIndex', 0).removeClass('is-invalid'),
        $('#TxOnExp').val('0.00'), $('#TxRemark').val(''),
        $('#TxAmount').val("0.00").prop('disabled', false).removeClass('is-invalid');
    ...
    if ($('#TxType').val() == "TAX") {
        if ($(this).prop('selectedIndex') != 0) {
            ...
            if ($(this).val() !== "25") {
                ...
                $('#TxAdInTxable').val('Y').prop('disabled', false).trigger('change');
                // $('#TxRefundable').prop('selectedIndex', 0);
                $('#TxRefundable').val('N').trigger('change');
...
$('.btnAddTax2Grid').click(function (e) {
    ...
    var isErr = 0;
    if ($('#TxType').val() == "TAX") {
        if (isTCSTaxAlreadyInGrid()) {
            $.notify('Please Clear TCS Tax First then add GST..!', { position: "top center", className: "error" });
            return false;
        }
    ...
    if ($('#TxType').val() == "EXPENSES") {
        ...
        if ($('#TxAccountCode option:selected').text() == 'Discount') {
            if (!$('#TxAmount').val() || $('#TxAmount').val() >= 0) {
                $.notify('Please Enter Negative Amount for Discount..!', { position: "top center", className: "error" });
                AddErrorCls('TxAmount');
                isErr = 1;
            }
            else {
            ...
            if (!$('#TxAmount').val() || $('#TxAmount').val() <= 0) {
                $.notify('Please Enter Positive Amount..!', { position: "top center", className: "error" });
                AddErrorCls('TxAmount');
                isErr = 1;
            }
            else {
        ...
        if ($('title').text().indexOf('SaleInvoice') === 0) {
            //if (isTaxAlreadyInGrid() && $("#TxAdInTxable").val()=='Y') {
            //    $.notify('Please Clear Tax First then add Expense..!', { position: "top center", className: "error" });
            //    return false;
            //}
            if (isTCSTaxAlreadyInGrid()) {
                $.notify('Please Clear TCS Tax First then add GST..!', { position: "top center", className: "error" });
...
function isTaxAlreadyInGrid() {
    return $('#divTaxGrid tr').filter(function () {
        return $(this).find('td').eq(2).text().trim().toUpperCase() === "TAX";
    }).length > 0;
}
function isTCSTaxAlreadyInGrid() {
    return $('[id^="txGridType-"]').filter(function () {
        return $(this).text().trim().toUpperCase() === "TCS";
    }).length > 0;
}

function clearTaxRows() {
    $('#divTaxGrid tr').each(function () {
        let typeText = $(this).find('td').eq(2).text().trim().toUpperCase();
        if (typeText === "TAX") {
            $(this).remove();
        }
    });
    ...
    $.ajax({
        ...
        success: function (resp, status, error) {
            if (resp.trim() == "") {
                ...
                //bymayuri
                if ($('title').text().indexOf('Purchase Rejection') == 0) {
                    INA = parseFloat($('#ItemNetAmount').val());
                }
            ...
            ClearTax();
            $('#TxType').val('TAX').trigger('change');
            // $('#TotalRoundOff').val('Y').trigger('change');
            /*}*/
        },
            ...
            $.ajax({
                ...
                success: function (data, status, xhr) {
                    ...
                    if (data.trim() != "") {
                        ...
                        //03/09/2025 - Dharmee's Changes for tax Net Amount suppose ItemAmount + TaxAmount
                        var INA = parseFloat($('#lblItemNetAmount').text());
                        if ($('title').text().indexOf('Purchase Rejection') == 0 || $('title').text().indexOf('Credit Note')==0) {
                             INA = parseFloat($('#ItemNetAmount').val());
                        }
                        var TTA = parseFloat($('#TotalTaxAmt').val());
                        var NT = INA + TTA;
                        $('#NetTotal').val((Math.round(NT * 100) / 100).toFixed(2));
                        ...
                        if ($("#TotalRoundOff").val() == "Y") {
                            ...
                            RoundOffAmt = Amt;
                            RoundOffAmt = (Amt - Math.round(Amt));
                            $('#TotalRoundOffAmt').val((Math.round(Amt) - Amt).toFixed(2));
                            Amt = Math.round(Amt);
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            ...
            if (data.trim() != "") {
                ...
                $('#TxType').val('TAX').trigger('change');
                var taxRowCount = $('#divTaxGrid tr').length;
                var lableitemnetamount = parseFloat($('#lblItemNetAmount').text());
                var ItemNetAmount= parseFloat($('#ItemNetAmount').val());
                var txSum = 0;
                if ($('#divTaxGrid tr td').length > 1) {
                    for (var i = 1; i <= taxRowCount; i++) {
                        txSum += parseFloat($('#txGridAmount-' + i).text() == '.00' ? 0 : $('#txGridAmount-' + i).text());
                ...
                $('#TotalTaxAmt').val(txSum);
                var INA = lableitemnetamount;
                var TTA = parseFloat($('#TotalTaxAmt').val());
                $('#ItemNetAmount').val(INA);
                var NT = INA + TTA;

                let roundedNetTotal = Math.round(parseFloat(NT || 0));

                $('#NetTotal').val(roundedNetTotal);
                
                $('#BaseNetTotal').val(roundedNetTotal);
                $('#AdjPendAmt').val(roundedNetTotal);
            }
            if ($('title').text().indexOf('Job Work') == 0) {
...
$('#TxRoundOff').change(function () {
    ...
    if ($(this).val() == "Y") {
        ...
        else if ($("#TxTaxType option:selected").text()=="TCS") {
            $.ajax({
                type: "POST",
                url: "/Tax/CalculateTCS",
                data: { "PC": "", "TP": $('#TxPercentg').val(), 'SN': SN },
                dataType: "json",
                success: function (data, status, error) {
        ...
        if ($('#TxType').val() == "TAX") {
            if ($("#TxTaxType option:selected").text() == "TCS") {
                $.ajax({
                    type: "POST",
                    url: "/Tax/CalculateTCS",
                    data: { "PC": "", "TP": $('#TxPercentg').val(), 'SN': SN },
...
$('#TotalRoundOff').change(function () {
    ...
    if (TxPageName.indexOf('Sale Rejection') == 0) { TxPageData = 'PurchaseRejection'; }
    if (TxPageName.indexOf('Sale Invoice') == 0) { TxPageData = 'SaleInvoice'; }
    ...
    if ($.isNumeric($('#NetTotal').val()) && parseFloat($('#NetTotal').val()) > 0) {
        if ($(this).val() == "Y") {
            ...
            //}
           // ClearDRCRGrid();
            if (TxPageName.indexOf('Purchase Bill') == 0) {
                onInvoiceChange('InvNo');
            } else if (TxPageName.indexOf('SaleInvoice')==0) {
                onInvoiceChange('SaleBillNo');
            } else if ($('title').text().indexOf('Purchase Rejection') ==0) {
            ...
            $("#RoundOff").hide();
            $('#RoundOffAccountCode').val(0).trigger('change');
            ...
            var INA = parseFloat($('#ItemNetAmount').val()) > 0 ? parseFloat($('#ItemNetAmount').val()) : (parseFloat($('#ItemNetAmount').text()) > 0 ? parseFloat($('#ItemNetAmount').text()) : 0);
                var TTA = parseFloat($('#TotalTaxAmt').val()) || 0;

            if ($('title').text().indexOf('SaleInvoice') == 0 || $('title').text().indexOf('DirectPurchaseBill') == 0) {
                INA = parseFloat($('#lblItemNetAmount').val()) > 0 ? parseFloat($('#lblItemNetAmount').val()) : (parseFloat($('#lblItemNetAmount').text()) > 0 ? parseFloat($('#lblItemNetAmount').text()) : 0);
            }
            else if ($('title').text().indexOf('Purchase Rejection') == 0) {
                INA = parseFloat($('#lblItemGridNetAmount').val()) > 0 ? parseFloat($('#lblItemGridNetAmount').val()) : (parseFloat($('#lblItemGridNetAmount').text()) > 0 ? parseFloat($('#lblItemGridNetAmount').text()) : 0);
            }
                var txSum = 0;
                var taxRowCount = $('#divTaxGrid tr').length;
                if ($('#divTaxGrid tr td').length > 1) {
            ...
            if (disPerc > 0) {
                ...
                NT = NT - disNT;
                $('#NetTotal').val((Math.round(NT * 100) / 100).toFixed(2));
                $('#BaseNetTotal').val((Math.round(NT * 100) / 100).toFixed(2));
                //comment by mayuri
                //$('#NetTotal').val(NT.toFixed(2));
               // $('#BaseNetTotal').val(NT.toFixed(2));
            }
...
async function ApplyHSNTaxForSaleRejection() {
    ClearAdjustmentGrid();
    var TxPageName = $('title').text();
    if ($('#TxType').val() == 'TAX' && $('#TxPartCode').prop('selectedIndex') == 0 && $('#TxItemCode').prop('selectedIndex') == 0) {
        var TxPageData;
         ...
         if (TxPageName.indexOf('Sale Rejection') == 0) {
            TxPageData = 'SaleRejection';
        }
    ...
    if ($('#TxType').val() == 'TAX' && $('#TxPartCode').prop('selectedIndex') == 0 && $('#TxItemCode').prop('selectedIndex') == 0) {
        ...
        if (objData.indexOf('DeleteTaxRow(1)') !== -1) {
            ...
            if (objData.trim()) {
                ...
                $('#BaseNetTotal').val(roundedNetTotal);
                $('#AdjPendAmt').val(roundedNetTotal);
                //$('#NetTotal').val(NT.toFixed(0));
                //$('#BaseNetTotal').val(NT.toFixed(0));
                //$('#AdjPendAmt').val(parseFloat($('#NetTotal').val() || 0).toFixed(2));
                if (TxPageData == 'PurchaseBill') {
                    onInvoiceChange('InvNo');
                }
...
async function GetDbCrDetail() {
    ...
    if (DbCrPageName.indexOf('Sale Order') === 0) {
        ...
        RoundOffAccountCode = $('#RoundOffAccountCode').val();
        RoundOffAmount = $('#TotalRoundOffAmt').val() != '' ? parseFloat($('#TotalRoundOffAmt').val()) : 0;
    }
        ...
        NetAmt = $('#NetTotal').val() != '' ? parseFloat($('#NetTotal').val()) : 0;
        RoundOffAccountCode = $('#RoundOffAccountCode').val();
        CashDis = $('#CashDisRs').val();
        AdditionalDis = $('#AdditionalDiscount').val();
        RoundOffAmount = $('#TotalRoundOffAmt').val() != '' ? parseFloat($('#TotalRoundOffAmt').val()) : 0;
    }
    ...
    if (DbCrPageData != null && DbCrPageData != undefined && DbCrPageData != '') {
        $.ajax({
            ...
            type: "POST",
            data: { "PageName": DbCrPageName, "docAccountCode": docAccountCode, "AccountCode": AccountCode, "BillAmt": BillAmt, "NetAmt": NetAmt, "RoundOffAccountCode": RoundOffAccountCode, "RoundOffAmount": RoundOffAmount, "CashDis": CashDis, "AdditionalDis": AdditionalDis },
            async: false,
...
function ClearAdj() {
    ...
    $('#AdjModeOfAdjstment').prop('selectedIndex', 0).removeClass('is-invalid'),
...
function DisplayAdjustmentRefDetail() {
    ...
    else if (DbCrPageName.indexOf('SaleRejection') == 0) {
        yearCode = $('#SaleRejYearCode').val();
        DRCR = "DR";
        VouchDate = $('#SaleRejEntryDate').val();
    }
...
/* ===================================================================================
   MULTI–LEVEL DRILLDOWN MANAGER (FINAL RESTORE–SAFE VERSION)
   Fixed:
   ✔ Monthly → Transaction → Back → Monthly restores original filters
   ✔ Transaction page cannot overwrite original Monthly filters
   ✔ Force ReportType from data-targetform
   ✔ Unlimited navigation stack
   ✔ Safe Select2 / datepickers / textboxes restore
=================================================================================== */

var isAutoChangingReportType = false;

const DD_STACK_KEY = "DD_STACK";
const DD_FILTER_PREFIX = "DD_FILTER_";
const DD_FLAG_RESTORE = "DD_ALLOW_RESTORE";
const DD_FLAG_DRILLDOWN = "DD_DRILLDOWN_NAVIGATION";

/* ---------------- STACK HELPERS ---------------- */
function ddGetStack() {
    return JSON.parse(sessionStorage.getItem(DD_STACK_KEY)) || [];
}

function ddSaveStack(stack) {
    sessionStorage.setItem(DD_STACK_KEY, JSON.stringify(stack));
}

function ddPush(info) {
    const stack = ddGetStack();
    stack.push(info);
    ddSaveStack(stack);
}
...
function ddHasStack() {
    return ddGetStack().length > 0;
}

/* ---------------- FILTER SAVE/LOAD ---------------- */

function ddSaveFilters(pageName, filters) {
    sessionStorage.setItem(DD_FILTER_PREFIX + pageName, JSON.stringify(filters));
}

function ddLoadFilters(pageName) {
    const raw = sessionStorage.getItem(DD_FILTER_PREFIX + pageName);
    return raw ? JSON.parse(raw) : null;
}

/* ---------------- MAIN DRILLDOWN ---------------- */
function callDrillDown(el, pageName, filterObj) {

    pageName = pageName || "DEFAULT_FORM";
    filterObj = filterObj || {};

    // prevent next page from saving its filters (fix for Monthly → Transaction)
    sessionStorage.setItem(DD_FLAG_DRILLDOWN, "1");

    // Save filters of current page
    ddSaveFilters(pageName, filterObj);

    const data = $(el).data();

    ddPush({
        returnPage: window.location.href,
        returnPageName: pageName,
        forceReportType: data.targetform || "",
        drillData: data
    });
...
/* ---------------- PAGE INITIALIZATION ---------------- */
function ddInitPage(pageName, backBtnId, reportDDLId) {

    // SHOW/HIDE BACK BUTTON
    if (backBtnId) {
        ddHasStack() ? $("#" + backBtnId).show() : $("#" + backBtnId).hide();
    }
    ...
    // RESTORE FILTERS ONLY WHEN BACK, NOT drilldown forward
    if (sessionStorage.getItem(DD_FLAG_RESTORE) === "1") {
        const savedFilters = ddLoadFilters(pageName);

        if (savedFilters) {
            for (let id in savedFilters) {
                const $el = $("#" + id);
                if (!$el.length) continue;

                if ($el.attr("type") === "checkbox") {
                    $el.prop("checked", savedFilters[id]);
                } else if ($el.hasClass("hasDatepicker")) {
                    $el.datepicker("setDate", savedFilters[id]);
                } else {
                    $el.val(savedFilters[id]);
                }
    ...
    // CLEAN FLAGS
    sessionStorage.removeItem(DD_FLAG_RESTORE);
    sessionStorage.removeItem(DD_FLAG_DRILLDOWN);

    // AUTO–SET REPORT TYPE IF NEEDED
    if (last.forceReportType && reportDDLId) {
        isAutoChangingReportType = true;
        $("#" + reportDDLId).val(last.forceReportType).trigger("change");
        setTimeout(() => isAutoChangingReportType = false, 0);
    }
}
...
function ddBack() {
    const last = ddPop();
    if (last && last.returnPage) {
        // allow restore on previous page
        sessionStorage.setItem(DD_FLAG_RESTORE, "1");
        window.location.href = last.returnPage;
    }
}


function calculateroundoffnettotal()
{
        var nettotal = parseFloat($('#NetTotal').val()) || 0;
        var roundoffnettotal = Math.round(nettotal);
    $('#NetTotal').val(roundoffnettotal.toFixed(2));

    var roundoffamount = roundoffnettotal - nettotal;
    $('#TotalRoundOffAmt').val(roundoffamount.toFixed(2));
