...
const DrilldownManager = (() => {
    ...
    function restoreFormFilters(formName) {
        ...
        for (const [id, value] of Object.entries(filters)) {
            ...
            // Trigger proper update for select2 or normal fields
            if ($el.hasClass("select2") || $el.hasClass("custom-select2")) {
    ...
    function goBack(rootUrl = "/") {
        ...
        const prevForm = prev.currentForm;
        const prevUrl = prev.url || rootUrl;

        // ✅ If previous URL is same page → just restore filters
        if (window.location.href === prevUrl ||
            location.pathname === new URL(prevUrl, window.location.origin).pathname) {
            restoreFormFilters(prevForm);

            // Restore ReportType if needed
            if ($('#ReportType').val() !== prevForm) {
    ...
    const api = {
        ...
        restoreIfAvailable: (formName) => {
            const raw = loadFormFilters(formName);
            return raw || null;
        }
    };
