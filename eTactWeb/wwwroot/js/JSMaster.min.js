...
const DrilldownManager = (() => {
    ...
    function getStack() { return JSON.parse(sessionStorage.getItem(STACK_KEY) || "[]"); }
    function setStack(list) { sessionStorage.setItem(STACK_KEY, JSON.stringify(list || [])); }

    function pushNavigation(entry) {
        const s = getStack(); s.push(entry); setStack(s);
    }
    ...
    function restoreFormFilters(formName) {
        ...
        for (const id in filters) {
            ...
            if ($el.attr("type") === "checkbox") $el.prop("checked", filters[id]);
            else if ($el.hasClass("hasDatepicker")) $el.datepicker("setDate", filters[id]);
            else $el.val(filters[id]);
    ...
    function goBack(root = "/") {
        const s = getStack();
        if (!s.length) return window.location.href = root;
    ...
    function toggleBackButton(selector = "#BackButton") {
        const s = getStack();
        s.length > 1 ? $(selector).show() : $(selector).hide();
    }
...
function collectFilters() {
    ...
    $("[id]").each(function () {
        ...
        filters[id] = ($(this).attr("type") === "checkbox")
            ? $(this).is(":checked")
            : $(this).val();
    });
    return filters;
...
window.callDrillDown = function (elem) {
    ...
    sessionStorage.setItem("DRILL__CLICK", clickname);
