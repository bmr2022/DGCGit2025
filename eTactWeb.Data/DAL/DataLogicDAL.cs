...
namespace eTactWeb.Data.DAL
{
    public class DataLogicDAL
    {
        ...
        public async Task<ResponseResult> GetDbCrDataGrid(DataTable DbCrGridd, DataTable TaxDetailDT, DataTable TDSDetailDT, string FormName,int? docAccountCode, int? AccountCode, decimal? ItemNetAmount, decimal? NetTotal ,int? RoundOffAccountCode, decimal? RoundOffAmount, decimal? CashDis, decimal? AdditionalDis)
        {
            var _ResponseResult = new ResponseResult();
            try
            {
                ...
                SqlParams.Add(new SqlParameter("@CashDis", CashDis));
                SqlParams.Add(new SqlParameter("@AdditionalDis", AdditionalDis));
                    ...
                    if (oDataSet.Tables.Count != 0 && oDataSet.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow row in oDataSet.Tables[0].Rows)
                        {
                            ...
                            _List.Add(new AdjustmentModel
                            {
                                ...
                                AdjAgnstPendAmt = !string.IsNullOrEmpty(row["BillNetAmt"].ToString()) ? Convert.ToDecimal(row["BillNetAmt"].ToString()) : 0,
                                AdjAgnstRemainingAmt = !string.IsNullOrEmpty(row["RemainingBalanceAmt"].ToString()) ? Convert.ToDecimal(row["RemainingBalanceAmt"].ToString()) : 0,
                                AdjAgnstAdjstedAmt = !string.IsNullOrEmpty(row["ADJUSTEDAMT"].ToString()) ? Convert.ToDecimal(row["ADJUSTEDAMT"].ToString()) : 0,
