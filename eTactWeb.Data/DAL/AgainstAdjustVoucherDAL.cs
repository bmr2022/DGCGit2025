    ...
    public class AgainstAdjustVoucherDAL
    {
        ...
        public async Task<ResponseResult> GetFormRights(int userId)
        {
            ...
            try
            {
                ...
                SqlParams.Add(new SqlParameter("@MainMenu", "AgainstAdjustVoucher"));
        ...
        public async Task<ResponseResult> SaveBankReceipt(AgainstAdjustVoucherModel model, DataTable GIGrid)
        {
            var _ResponseResult = new ResponseResult();
                        ...
                        if (oDataSet.Tables.Count > 0)
                        {
                            foreach (DataRow row in oDataSet.Tables[0].Rows)
                            {
                                var model = new AgainstAdjustVoucherModel
                                {
                                    ...
                                    CrAmt = Convert.ToDecimal(row["CrAmt"]),
                                    VoucherBillAmt = Convert.ToDecimal(row["BillNetAmt"]),
                                    AdjustmentAmt = Convert.ToDecimal(row["AdjustmentAmt"]),
                                    AdjustmentAmtOthCur = Convert.ToDecimal(row["AdjustmentAmt"]),
                                    ModeOfAdjustment = row["ModOfAdjust"].ToString(),
                                    AgainstVoucherEntryId = Convert.ToInt32(row["AgainstAccEntryId"]),
                                    AgainstVoucherNo = row["AgainstVoucherNo"].ToString(),
                                    AgainstVoucherType = row["AgainstvoucherType"].ToString(),
        ...
        public async Task<AgainstAdjustVoucherModel> GetDashBoardDetailData(string FromDate, string ToDate, string LedgerName, string VoucherNo, string AgainstBillno, string AgainstVoucherNo)
        {
            ...
            try
            {
                ...
                //if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
                //{
                //    model.AgainstAdjustVoucherList = (from DataRow dr in oDataSet.Tables[0].Rows
                //                                select new AgainstAdjustVoucherModel
                //                                {
        ...
        public async Task<AgainstAdjustVoucherModel> GetDashBoardSummaryData(string FromDate, string ToDate, string LedgerName, string VoucherNo, string AgainstBillno, string AgainstVoucherNo)
        {
            DataSet? oDataSet = new DataSet();
            var model = new AgainstAdjustVoucherModel();
            try
        ...
        public async Task<AgainstAdjustVoucherModel> PopUpForPendingVouchers(PopUpDataTableAgainstRef DataTable)
        {
            DataSet? oDataSet = new DataSet();
            var model = new AgainstAdjustVoucherModel();
            try
        ...
        private static AgainstAdjustVoucherModel PrepareView(DataSet DS, ref AgainstAdjustVoucherModel? model)
        {
            var ItemList = new List<AgainstAdjustVoucherModel>();
            ...
            if (DS.Tables.Count != 0 && DS.Tables[1].Rows.Count > 0)
            {
                ...
                {
                    ItemList.Add(new AgainstAdjustVoucherModel
                    {
                        ...
                        AdjustmentAmtOthCur = Convert.ToDecimal(row["AmountInOtherCurr"].ToString()),
               ...
               // model.AgainstAdjustVoucherList = ItemList.OrderBy(x => x.SrNO).ToList();
            }
