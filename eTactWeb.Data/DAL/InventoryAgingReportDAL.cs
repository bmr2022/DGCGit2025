                    ...
                    if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
                    {
                        resultList.InventoryAgingReportGrid = (from DataRow row in oDataSet.Tables[0].Rows
                                                            select new InventoryAgingReportModel
                                                            {
                                                                ...
                                                                Aging_0_30 = row["0-30"] != DBNull.Value ? Convert.ToDecimal(row["0-30"]) : 0,
                                                                Aging_31_60 = row["31-60"] != DBNull.Value ? Convert.ToDecimal(row["31-60"]) : 0,
                                                                Aging_61_90 = row["61-90"] != DBNull.Value ? Convert.ToDecimal(row["61-90"]) : 0,
                                                                Aging_91 = row[">=91"] != DBNull.Value ? Convert.ToDecimal(row[">=91"]) : 0,
