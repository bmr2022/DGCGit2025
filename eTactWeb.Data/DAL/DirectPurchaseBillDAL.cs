                ...
                if (oDataSet.Tables.Count != 0 && oDataSet.Tables[1].Rows.Count > 0)
                {
                    foreach (DataRow row in oDataSet.Tables[1].Rows)
                    {
                        _ItemList.Add(new DPBItemDetail
                        {
                            ...
                            CGSTPer = (!string.IsNullOrEmpty(row["CGSTPer"].ToString())) ? Convert.ToDecimal(row["CGSTPer"]) : 0,
                            SGSTPer = (!string.IsNullOrEmpty(row["SGSTPer"].ToString())) ? Convert.ToDecimal(row["SGSTPer"]) : 0,
                            IGSTPer = (!string.IsNullOrEmpty(row["IGSTPer"].ToString())) ? Convert.ToDecimal(row["IGSTPer"]) : 0,
                            CGSTAmt = (!string.IsNullOrEmpty(row["ItemCGSTAmt"].ToString())) ? Convert.ToDecimal(row["ItemCGSTAmt"]) : 0,
                            SGSTAmt = (!string.IsNullOrEmpty(row["ItemSGSTAmt"].ToString())) ? Convert.ToDecimal(row["ItemSGSTAmt"]) : 0,
                            IGSTAmt = (!string.IsNullOrEmpty(row["ItemIGSTAmt"].ToString())) ? Convert.ToDecimal(row["ItemIGSTAmt"]) : 0,
