...
namespace eTactWeb.Data.DAL
{
	public class SaleRejectionDAL
	{
		...
		public SaleRejectionDAL(IConfiguration configuration, IDataLogic iDataLogic, ConnectionStringService connectionStringService, ICommon common)
		{
		...
		public async Task<ResponseResult> GetTotalAmount(SaleRejectionFilter model)
		{
			...
			try
			{
				...
				SqlParams.Add(new SqlParameter("@AgainstBillNo", model.AgainstBillNo));
		...
		public async Task<ResponseResult> GetSaleRejectionDashboard(string fromDate, string toDate, string custInvoiceNo, int AccountCode, string mrnNo, string gateNo, int ItemCode, string againstBillNo, string voucherNo, string summaryDetail, string searchBox)
		{
	...
	{
		{ "@CustInvoiceNo", EmptyIfNull(custInvoiceNo) },
		{ "@AccountCode", AccountCode},
		{ "@MrnNo", EmptyIfNull(mrnNo) },
		{ "@GateNo", EmptyIfNull(gateNo) },
		{ "@itemCode", ItemCode },
		{ "@AgainstBillNo", EmptyIfNull(againstBillNo) },
		{ "@DocTypeAccountCode", EmptyIfNull(againstBillNo) },
		{ "@VoucherNo", EmptyIfNull(voucherNo) },
		...
		public async Task<ResponseResult> NewEntryId(int YearCode, string SaleRejEntryDate, string SubVoucherName)
		{
			...
			try
			{
				...
				SqlParams.Add(new SqlParameter("@SaleRejEntryDate", ParseFormattedDate(SaleRejEntryDate)));
				_ResponseResult = await _IDataLogic.ExecuteDataSet("AccSpSalerejection", SqlParams);
			}
			catch (Exception ex)
			{
		...
		private static SaleRejectionModel PrepareView(DataSet DS, ref SaleRejectionModel? model, string Mode)
		{
			...
			model.CurrencyName = DS.Tables[0].Rows[0]["Currency"]?.ToString();
			...
			model.SaleRejectionSubVoucherNo = DS.Tables[0].Rows[0]["VoucherNo"]?.ToString();
			...
			if (DS.Tables.Count != 0 && DS.Tables[4].Rows.Count > 0)
			{
				...
				foreach (DataRow row in DS.Tables[4].Rows)
				{
					adjustGrid.Add(new AdjustmentModel
					{
						...
						AdjAgnstModeOfAdjstmentName = row["ModOfAdjust"]?.ToString(),
						AdjAgnstNewRefNo = row["AgainstVoucherNo"]?.ToString(),
		...
		internal async Task<ResponseResult> SaveSaleRejection(SaleRejectionModel model, DataTable SBGrid, DataTable TaxDetailDT, DataTable DrCrDetailDT, DataTable AdjDetailDT)
		{
			...
			try
			{
				...
				SqlParams.Add(new SqlParameter("@GateEntryId", model.GateEntryId));
				...
				SqlParams.Add(new SqlParameter("@SubVoucherNo", model.SaleRejectionSubVoucherNo ?? string.Empty));
		...
		private static SaleRejectionModel PrepareMRNView(DataSet DS, ref SaleRejectionModel? model)
		{
			...
			// ----------------- HEADER DATA -----------------
			model.GateNo = DS.Tables[0].Rows[0]["GateNo"]?.ToString();
			model.GateEntryId = Convert.ToInt32(DS.Tables[0].Rows[0]["GateEntryId"]);
			model.Gateyearcode = Convert.ToInt32(DS.Tables[0].Rows[0]["GateYearCode"]);
			model.CurrencyId = Convert.ToInt32(DS.Tables[0].Rows[0]["CurrencyId"]);
			model.CurrencyName = DS.Tables[0].Rows[0]["Currency"]?.ToString();
			model.GateDate = DS.Tables[0].Rows[0]["GateDate"]?.ToString();
			...
			if (DS.Tables.Count > 1 && DS.Tables[1].Rows.Count > 0)
			{
				foreach (DataRow row in DS.Tables[1].Rows)
				{
					int saleBillQty = row["SaleBillQty"] != DBNull.Value ? Convert.ToInt32(row["SaleBillQty"]) : 0;
					int saleRate = row["SaleRate"] != DBNull.Value ? Convert.ToInt32(row["SaleRate"]) : 0;

					int rejQty = row["RejQty"] != DBNull.Value ? Convert.ToInt32(row["RejQty"]) : 0;
					int rejRate = row["RejRate"] != DBNull.Value ? Convert.ToInt32(row["RejRate"]) : 0;
					int amt = row["RejAmount"] != DBNull.Value ? Convert.ToInt32(row["RejAmount"]) : 0;

					// 🔥 Pending condition
					if (model.BillAmt == 0 && model.InvNetAmt == 0)
					{
						rejQty = saleBillQty;
						rejRate = saleRate;
						amt = saleRate;
					...
					SaleRejectionGrid.Add(new SaleRejectionDetail
					{
						...
						SaleBillQty = saleBillQty,
						Rate = saleRate,

						RejQty = rejQty,       // ✅ applied
						RejRate = rejRate,     // ✅ applied
        ...
        public async Task<List<SaleRejectionDetail>> GetSaleBillData(string saleBillNo,int AccountCode)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("AccSpSalerejection", con))
                    {
                        ...
                        using (SqlDataReader dr = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            {
                                list.Add(new SaleRejectionDetail
                                {
                                    AgainstBillEntryId = dr["SaleBillEntryId"] != DBNull.Value ? Convert.ToInt64(dr["SaleBillEntryId"]) : 0,
                                    AgainstBillYearCode = dr["SaleBillYearCode"] != DBNull.Value ? Convert.ToInt64(dr["SaleBillYearCode"]) : 0,
                                    AgainstBillNo = dr["SaleBillNo"]?.ToString(),
                                    ...
                                    IGSTAmt = dr["IGSTAmount"] != DBNull.Value ? Convert.ToDecimal(dr["IGSTAmount"]) : 0,

                                    CGSTPer = dr["CGST"] != DBNull.Value ? Convert.ToDecimal(dr["CGST"]) : 0,
                                    SGSTPer = dr["SGST"] != DBNull.Value ? Convert.ToDecimal(dr["SGST"]) : 0,
                                    IGSTPer = dr["IGST"] != DBNull.Value ? Convert.ToDecimal(dr["IGST"]) : 0,

                                    Unit = dr["Unit"]?.ToString(),
                                    HSNNo = dr["HSNNO"] != DBNull.Value ? Convert.ToInt32(dr["HSNNO"]) : 0
                                });
                            }
                        }
                    }
            ...
            {
                // You can log this in file or DB also
                throw new Exception("Error in GetSaleBillData DAL: " + ex.Message, ex);
            }
