...
namespace eTactWeb.Data.DAL
{
    ...
    public async Task<ResponseResult> GetItemDetail(string Partcode)
    {
                ...
                if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
                {
                    resultList.ItemDetailGrid = (from DataRow row in oDataSet.Tables[0].Rows
                                                 select new SaleBillDetail
                                                 {
                                                     ...
                                                     Qty = row["BillQty"] == DBNull.Value ? 0 : Convert.ToDecimal(row["BillQty"]),
                                                     Rate = row["BillRate"] == DBNull.Value ? 0 : Convert.ToDecimal(row["BillRate"]),
                                                     DiscountPer = row["DiscountPer"] == DBNull.Value ? 0 : Convert.ToDecimal(row["DiscountPer"]),
                ...
                if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
                {
                    resultList.ItemDetailGrid = (from DataRow row in oDataSet.Tables[0].Rows
                                                 select new SaleBillDetail
                                                 {
                                                     ...
                                                     DiscountPer = row["SaleDiscount"] == DBNull.Value ? 0 : Convert.ToDecimal(row["SaleDiscount"]),
                                                    LotStock = row["BatchStock"] == DBNull.Value ? 0 : Convert.ToDecimal(row["BatchStock"]),
        ...
        public async Task<ResponseResult> FillVehicleNo()
        {
            ...
            {
                var SqlParams = new List<dynamic>();
                SqlParams.Add(new SqlParameter("@Flag", "FillVehicleNo"));
                _ResponseResult = await _IDataLogic.ExecuteDataTable("SP_SaleBillMainDetail", SqlParams);
            }
            catch (Exception ex)
            {
        ...
        private static SaleBillModel PrepareView(DataSet DS, ref SaleBillModel? model, string Mode)
        {
            try
            {
                ...
                model.BillAmt = Convert.ToDecimal(DS.Tables[0].Rows[0]["BillAmt"]);
                model.BillAmtWord = DS.Tables[0].Rows[0]["BillAmtWord"]?.ToString();
                model.TaxableAmt = Convert.ToDecimal(DS.Tables[0].Rows[0]["TaxableAmt"]);
                model.TaxbaleAmtInWord = DS.Tables[0].Rows[0]["TaxbaleAmtInWord"]?.ToString();
                model.GSTAmount = Convert.ToDecimal(DS.Tables[0].Rows[0]["GSTAmount"]);
                ...
                model.DiscountAmt = Convert.ToDecimal(DS.Tables[0].Rows[0]["DiscountAmt"]);
                model.INVNetAmt = Convert.ToDecimal(DS.Tables[0].Rows[0]["INVNetAmt"]);
                model.NetTotal = Convert.ToDecimal(DS.Tables[0].Rows[0]["INVNetAmt"]);
                ...
                model.CashDisPer = Convert.ToDecimal(DS.Tables[0].Rows[0]["CashDisPer"]);
                model.CashDisRs = Convert.ToDecimal(DS.Tables[0].Rows[0]["CashDisRs"]);
                ...
                model.currExchangeRate = Convert.ToDecimal(DS.Tables[0].Rows[0]["ExchangeRate"]);
                ...
                if (DS.Tables.Count != 0 && DS.Tables[1].Rows.Count > 0)
                {
                    foreach (DataRow row in DS.Tables[1].Rows)
                    {
                        SaleBillGrid.Add(new SaleBillDetail
                        {
                            ...
                            OriginalMRP = row["OriginalMRP"] != DBNull.Value ? Convert.ToDecimal(row["OriginalMRP"]) : 0,
                            SOPendQty = row["SOPendQty"] != DBNull.Value ? Convert.ToDecimal(row["SOPendQty"]) : 0,
                ...
                if (DS.Tables.Count != 0 && DS.Tables[5].Rows.Count > 0)
                {
                    foreach (DataRow row in DS.Tables[5].Rows)
                    {
                        customerJWAdj.Add(new CustomerJobWorkChallanAdj //
                        {
                            ...
                            AdjQty = row["AdjQty"] != DBNull.Value ? Convert.ToDecimal(row["AdjQty"]) : 0,
                            CC = row["CC"]?.ToString(),
                            UID = row["UID"] != DBNull.Value ? Convert.ToInt32(row["UID"]) : 0,
                            AdjFormType = row["AdjFormType"]?.ToString(),
                            TillDate = row["TillDate"]?.ToString(),
                            TotIssQty = row["TotIssQty"] != DBNull.Value ? Convert.ToDecimal(row["TotIssQty"]) : 0,
                            ...
                            ProcessID = row["ProcessID"] != DBNull.Value ? Convert.ToInt32(row["ProcessID"]) : 0,
                            BOMInd = row["BOMInd"]?.ToString(),
                            IssQty = row["IssQty"] != DBNull.Value ? Convert.ToDecimal(row["IssQty"]) : 0,
                            TotadjQty = row["TotadjQty"] != DBNull.Value ? Convert.ToDecimal(row["TotadjQty"]) : 0,
                            TotalIssQty = row["TotalIssQty"] != DBNull.Value ? Convert.ToDecimal(row["TotalIssQty"]) : 0,
                            TotalRecQty = row["TotalRecQty"] != DBNull.Value ? Convert.ToDecimal(row["TotalRecQty"]) : 0,
                            ...
                            IdealScrapQty = row["IdealScrapQty"] != DBNull.Value ? Convert.ToDecimal(row["IdealScrapQty"]) : 0,
                            IssuedScrapQty = row["IssuedScrapQty"] != DBNull.Value ? Convert.ToDecimal(row["IssuedScrapQty"]) : 0,
                            PreRecChallanNo = row["PreRecChallanNo"]?.ToString(),
                            ScrapqtyagainstRcvqty = row["ScrapqtyagainstRcvqty"] != DBNull.Value ? Convert.ToDecimal(row["ScrapqtyagainstRcvqty"]) : 0,
                ...
                foreach (DataRow row in DS.Tables[0].Rows)
                {
                    ItemGrid.AddRange(new List<CustomerJobWorkIssueAdjustDetail>
                        {
                            new CustomerJobWorkIssueAdjustDetail
                            {
                                 ...
                                 QtyToBeRec = Convert.ToDecimal(row["QtyToBeRec"]),
                                 ActualAdjQty = Convert.ToDecimal(row["ActualAdjQty"]),
                                 Batchno = row["batchno"].ToString(),
                                 UniqueBatchno = row["uniquebatchno"].ToString(),
                                 SeqNo = Convert.ToInt32(row["seqno"]),
                                 IssueQty = Convert.ToDecimal(row["IssueQty"]),
                                 Rate = Convert.ToDecimal(row["Rate"]),
                                 OriginalRecQty = Convert.ToDecimal(row["OriginalRecQty"]),
                    ...
                    custJobWorkChallanAdjGrid.AddRange(new List<CustomerJobWorkChallanAdj>
                    {
                        new CustomerJobWorkChallanAdj()
                        {
                            ...
                            TotIssQty = row["IssueQty"] != DBNull.Value ? Convert.ToDecimal(row["IssueQty"]) : 0,
                            PendQty = row["PendQty"] != DBNull.Value ? Convert.ToDecimal(row["PendQty"]) : 0 ,
                            BOMQty = row["BomQty"] != DBNull.Value ?  Convert.ToDecimal(row["BomQty"]) :0,
                            ...
                            IssQty = row["IssueQty"] != DBNull.Value ?  Convert.ToDecimal(row["IssueQty"]) : 0,
                            TotadjQty = row["ActualAdjQty"] != DBNull.Value ?  Convert.ToDecimal(row["ActualAdjQty"]) : 0,
                            TotalIssQty = row["OriginalRecQty"] != DBNull.Value ? Convert.ToDecimal(row["OriginalRecQty"]) : 0,
                            TotalRecQty = row["QtyToBeRec"] != DBNull.Value ? Convert.ToDecimal(row["QtyToBeRec"]) : 0,
                            ...
                            IdealScrapQty = row["IdealScrap"] != DBNull.Value ? Convert.ToDecimal(row["IdealScrap"]) : 0,
                            IssuedScrapQty = row["IssuedScrap"] != DBNull.Value ?  Convert.ToDecimal(row["IssuedScrap"]) : 0,
                            PreRecChallanNo = row["RecJWChallanNo"].ToString(),
                            ScrapqtyagainstRcvqty = row["ActualAdjQty"] != DBNull.Value ? Convert.ToDecimal(row["ActualAdjQty"]) : 0,
                    ...
                    bomItemGrid.AddRange(new List<BomCustomerJWIssChallanADJ>
                        {
                            new BomCustomerJWIssChallanADJ
                            {
                                 FGPartCode = row["FGPartCode"].ToString(),
                                  ...
                                  ActualPendQty = Convert.ToDecimal(row["ActualPendQty"]),
                                  PendToAdjust = Convert.ToDecimal(row["PendToadjust"]),
                                  BOMIND = row["BOMIND"].ToString(),
