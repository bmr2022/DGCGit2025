                ...
                if (DS.Tables.Count != 0 && DS.Tables[1].Rows.Count > 0)
                {
                    foreach (DataRow row in DS.Tables[1].Rows)
                    {
                        SaleBillGrid.Add(new SaleBillDetail
                        {
                            ...
                            Qty = row["BillQty"] != DBNull.Value ? Convert.ToSingle(row["BillQty"]) : 0,
                            UnitOfRate = row["UnitOfRate"]?.ToString(),
                            RateInOtherCurr = row["RateInOtherCurr"] != DBNull.Value ? Convert.ToInt32(row["RateInOtherCurr"]) : 0,
                            Rate = row["BillRate"] != DBNull.Value ? Convert.ToSingle(row["BillRate"]) : 0,
                            AltUnit = row["AltUnit"]?.ToString(),
                            AltQty = row["AltQty"] != DBNull.Value ? Convert.ToSingle(row["AltQty"]) : 0,
                            ItemWeight = row["ItemWeight"] != DBNull.Value ? Convert.ToSingle(row["ItemWeight"]) : 0,
                            NoofPcs = row["NoofPcs"] != DBNull.Value ? Convert.ToInt32(row["NoofPcs"]) : 0,
                            CustomerPartCode = row["CustomerPartCode"]?.ToString(),
                            MRP = row["MRP"] != DBNull.Value ? Convert.ToSingle(row["MRP"]) : 0,
                            OriginalMRP = row["OriginalMRP"] != DBNull.Value ? Convert.ToSingle(row["OriginalMRP"]) : 0,
                            SOPendQty = row["SOPendQty"] != DBNull.Value ? Convert.ToSingle(row["SOPendQty"]) : 0,
