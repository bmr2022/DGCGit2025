...
public class ProductionEntryDAL
{
    ...
    public async Task<ResponseResult> FillMachineGroup(int machineId)
    {
        ...
        try
        {
            ...
            SqlParams.Add(new SqlParameter("@MachineId", machineId));
            _ResponseResult = await _IDataLogic.ExecuteDataTable("SP_ProductionEntry", SqlParams);
        }
        catch (Exception ex)
        {
    ...
    public async Task<ResponseResult> FillMachineName(int groupId)
    {
        ...
        try
        {
            ...
            SqlParams.Add(new SqlParameter("@MachGroupId", groupId));
            _ResponseResult = await _IDataLogic.ExecuteDataTable("SP_ProductionEntry", SqlParams);
        }
        catch (Exception ex)
        {
    ...
    public async Task<ResponseResult> SaveProductionEntry(ProductionEntryModel model, DataTable GIGrid, DataTable BreakDownGrid, DataTable OperatorGrid, DataTable ScrapGrid, DataTable ProductGrid)
    {
        ...
        try
        {
            ...
            var prodDt = CommonFunc.ParseFormattedDate(model.ActualProdDate);
            ...
            SqlParams.Add(new SqlParameter("@TransferFGToWCorSTORE", (model.SenToQc != null && model.SenToQc.ToUpper() == "Y") ? "W" : (model.Store ?? "")));
            ...
            if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
            {
                model.ProductionDashboard = (from DataRow dr in oDataSet.Tables[0].Rows
                                             select new ProductionEntryDashboard
                                             {
                                                 ...
                                                 BOMDate = dr["BOMDate"] == DBNull.Value ? null : Convert.ToDateTime(dr["BOMDate"]).ToString("yyyy-MM-dd"),
            ...
            if (oDataSet.Tables.Count > 0 && oDataSet.Tables[0].Rows.Count > 0)
            {
                model.ProductionDashboard = (from DataRow dr in oDataSet.Tables[0].Rows
                                             select new ProductionEntryDashboard
                                             {
                                                 ...
                                                 RMPartCode = dr["RMPartCode"]?.ToString() ?? "",
                                                 RMItemName = dr["RMItemName"]?.ToString() ?? "",
                                                 ConsumedRMQTY = dr["ConsumedRMQTY"] != DBNull.Value ? Convert.ToInt32(dr["ConsumedRMQTY"]) : 0,
                                                 ConsumedRMUnit = dr["RMunit"]?.ToString() ?? "",
                                                 MainRMQTY = dr["MainRMQTY"] != DBNull.Value ? Convert.ToDecimal(dr["MainRMQTY"]) : 0,
                                                 MainRMUnit = dr["MainRMUnit"]?.ToString() ?? "",
                                                 TotalReqRMQty = dr["TotalReqRMQty"] != DBNull.Value ? Convert.ToDecimal(dr["TotalReqRMQty"]) : 0,
                                                 TotalStock = dr["TotalStock"] != DBNull.Value ? Convert.ToDecimal(dr["TotalStock"]) : 0,
                                                 AltRMQty = dr["AltRMQty"] != DBNull.Value ? Convert.ToDecimal(dr["AltRMQty"]) : 0,
                                                 AltRMUnit = dr["AltRMUnit"]?.ToString() ?? "",
                                                 RMNetWt = dr["RMNetWt"] != DBNull.Value ? Convert.ToDecimal(dr["RMNetWt"]) : 0,
                                                 GrossWt = dr["GrossWt"] != DBNull.Value ? Convert.ToDecimal(dr["GrossWt"]) : 0,
                                                 BOMNO = dr["BOMRevNO"] != DBNull.Value ? Convert.ToInt32(dr["BOMRevNO"]) : 0,
                                                 BOMRevDate = dr["BOMRevDate"]?.ToString() ?? "",
                                                 ManualAutoEntry = dr["ManualAutoEntry"]?.ToString() ?? "",
                                                 ...
                                                 QCMandatory = dr["QCMandatory"]?.ToString() ?? "",
                                                 PendQtyForQC = dr["PendQtyForQC"] != DBNull.Value ? Convert.ToDecimal(dr["PendQtyForQC"]) : 0
