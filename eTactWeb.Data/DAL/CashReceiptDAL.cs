...
{
    public class CashReceiptDAL
    {
        private readonly IDataLogic _IDataLogic;
        private readonly string DBConnectionString = string.Empty;
        private IDataReader? Reader;
        private readonly ConnectionStringService _connectionStringService;
        public CashReceiptDAL(IConfiguration configuration, IDataLogic iDataLogic, ConnectionStringService connectionStringService)
        {
        ...
        public async Task<ResponseResult> FillIntrument(string VoucherType)
        {
            ...
            try
            {
                ...
                SqlParams.Add(new SqlParameter("@VoucherType", "CashReceipt"));     
                _ResponseResult = await _IDataLogic.ExecuteDataTable("AccSpVoucherEntry", SqlParams);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseResult> SaveCashReceipt(CashReceiptModel model, DataTable GIGrid)
        {
            ...
            try
            {
                ...
                sqlParams.Add(new SqlParameter("@Subvoucher", "CASH-RECEIPT"));
        ...
        public async Task<CashReceiptModel> GetDashBoardDetailData(string FromDate, string ToDate, string LedgerName, string VoucherNo, string AgainstVoucherRefNo, string AgainstVoucherNo)
        {
            ...
            try
            {
                using (SqlConnection myConnection = new SqlConnection(DBConnectionString))
                {
                    ...
                    oCmd.Parameters.AddWithValue("@VoucherType", "Cash-Receipt");
                ...
                {
                    model.CashReceiptGrid = (from DataRow dr in oDataSet.Tables[0].Rows
        ...
        public async Task<CashReceiptModel> GetDashBoardSummaryData(string FromDate, string ToDate, string LedgerName, string VoucherNo, string AgainstVoucherRefNo, string AgainstVoucherNo)
        {
            DataSet? oDataSet = new DataSet();
            var model = new CashReceiptModel();
            try
        ...
        private static CashReceiptModel PrepareView(DataSet DS, ref CashReceiptModel? model)
        {
            var ItemList = new List<CashReceiptModel>();
            ...
            if (DS.Tables.Count != 0 && DS.Tables[1].Rows.Count > 0)
            {
                ...
                {
                    ItemList.Add(new CashReceiptModel
                    {
                ...
                model.CashReceiptGrid = ItemList.OrderBy(x => x.SrNO).ToList();
            }
